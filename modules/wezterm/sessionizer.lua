...
M.list = function(window, pane)
	...
	if not success then
		wezterm.log_error("Failed to fetch workspaces: " .. stderr)
		return
	end

	local output = wezterm.serde.json_decode(stdout)
	for _, entry in pairs(output) do
		local workspace = entry.workspace
		local parent, last = workspace:match("(.*/)([^/]+)$")
		if not parent then
			parent = ""
			last = workspace
		end
		...
		table.insert(workspaces, { label = label, id = tostring(workspace) })
	end
